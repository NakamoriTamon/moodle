...
try {
    ...
    // // 講義形式情報の取得
    // $lectureFormatTypes = [];
    // foreach ($lecture_format_ids as $format_id) {
    //     $stmt = $pdo->prepare("SELECT name FROM mdl_lecture_format WHERE id = :id");
    //     $stmt->execute([':id' => $format_id]);
    //     $formatName = $stmt->fetchColumn();
    //     if ($formatName) {
    //         $lectureFormatTypes[] = $formatName;
    //     }
    // }
    ...
    //         $eventDateText = $formattedDate . ' ' . $start_hour . '～' . $end_hour;
    //     }
    // } elseif ($event_kbn == PLURAL_EVENT) {
    //     // 複数回シリーズ
    //     reset($lectures);
    //     $firstKey = key($lectures);
    //     if (!empty($lectures[$firstKey]['course_date'])) {
    //         $date = new DateTime($lectures[$firstKey]['course_date']);
    //         $formattedDate = $date->format('Y年m月d日');
    //         $eventDateText = $formattedDate . ' ' . $start_hour . '～' . $end_hour . '（第1回目）';
    //     }
    // } elseif ($event_kbn == EVERY_DAY_EVENT) {
    //     // 毎日開催
    //     if (!empty($start_event_date) && !empty($end_event_date)) {
    //         $startDate = new DateTime($start_event_date);
    //         $endDate = new DateTime($end_event_date);
    //         $eventDateText = $startDate->format('Y年m月d日') . '～' . $endDate->format('Y年m月d日') . ' ' . $start_hour . '～' . $end_hour;
    //     }
    // }

    // // 会場情報の設定
    // $venueInfo = !empty($venue_name) ? $venue_name : (!empty($formatText) ? $formatText : 'オンライン');
    ...
    //         このたび、新たなイベント 「{$name}」 を開催する運びとなりましたので、ご案内申し上げます。</p>

    //         <div style=\"background-color: #f7f7f7; padding: 15px; margin: 20px 0; border-left: 5px solid #3366cc;\">
    ...
    //         <p>本イベントでは、最新の知見や実践的な内容をご紹介し、皆様のスキルアップや知識の向上に貢献できる内容をご用意しております。</p>

    //         <div style=\"text-align: center; margin: 30px 0;\">
    //             <a href=\"{$applyUrl}\" style=\"background-color: #3366cc; color: white; padding: 12px 24px; text-decoration: none; font-weight: bold; border-radius: 4px;\">▼お申し込みはこちら</a>
    //         </div>

    //         <p>なお、定員に限りがございますので、お早めのご登録をお願い申し上げます。<br>
    ...
    //     $mail->setFrom($_ENV['MAIL_FROM_ADRESS'], '知の広場 事務局');
    ...
    //         // 送信先がなければ管理者のみに送信
    //         $mail->addAddress($_ENV['ADMIN_EMAIL'], '管理者');
    //     } else {
    //         // 最大100名にのみBCCで送信（メール送信の負荷考慮）
    //         $count = 0;
    ...
    //     $mail->addReplyTo('no-reply@example.com', '知の広場（返信不可）');
    //     $mail->isHTML(true);
    //     $mail->Subject = "【新規イベント開催のご案内】{$name}";
    //     $mail->Body = $htmlBody;
    ...
    //     $mail->send();
    //     error_log('イベント登録メールの送信に成功しました。イベントID: ' . $eventId);
    // } catch (Exception $e) {
    //     var_dump($e);
    //     // メール送信に失敗してもトランザクションは既にコミット済みなので続行
    //     error_log('イベント登録メールの送信に失敗しました: ' . $e->getMessage());
    // }
