...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (empty($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
            ...
            header('Location: /custom/admin/app/Views/survey/custom_index');
            exit;
        }
    }

    $transaction = $DB->start_delegated_transaction();
    $DB->set_field('event_survey_customfield_category', 'is_delete', true, ['id' => $id]);
    ...
    try {
        ...
        $_SESSION['message_error'] = '削除に失敗しました';
        redirect('/custom/admin/app/Views/survey/custom_index.php');
        exit;
    }
}
